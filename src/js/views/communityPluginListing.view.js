...
export default async function communityPluginListingView({
  ...
  async function loadDetails() {
    ...
    await Promise.all([
      ...
      pluginService
        ...
        .catch((error) => console.error("Failed to load plugin README", error)),
    ]);
    state.$loading.set(false);
  }
  ...
  function installButtonTemplate({ listing, pendingAction }) {
    ...
    >
      ${pendingAction
        ? html`${pendingAction === "uninstall" ? "Uninstalling" : "Installing"}
              ...
              >
                ...
                <div class="plugin-listing-meta">
                  ${version
                    ...
                    : ""}
                  <div class="plugin-listing-author">By ${listing.author}</div>
                  ${listing.repo
