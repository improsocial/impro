...
describe("SystemNotificationService", () => {
  ...
  describe("app badge", () => {
    ...
    // Unlike the Notification API path, badging is not desktop-only: an
    // installed PWA is exactly where the badge is visible.
    it("badges on touch-only devices too", async () => {
      simulateTouchOnlyDevice();
      const notificationService = createMockNotificationService({
        numNotifications: 4,
      });
